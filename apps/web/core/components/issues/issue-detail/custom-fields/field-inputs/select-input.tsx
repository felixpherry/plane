    ...
    >
      ...
      >
        {BUTTON_VARIANTS_WITH_TEXT.includes(buttonVariant) && (
          <span className={cn("grow truncate text-left", !value && "text-placeholder")}>{value || placeholder}</span>
        )}
        {dropdownArrow && (
              ...
              {filteredOptions.length > 0 ? (
                filteredOptions.map((option) => (
                  ...
                  >
                    ...
                    {value === option && <CheckIcon className="text-custom-primary-100 h-3 w-3 shrink-0" />}
                  </Combobox.Option>
                ))
              ) : (
